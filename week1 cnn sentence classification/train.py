...
# Misc Parameters
tf.flags.DEFINE_boolean("allow_soft_placement", True, "Allow device soft device placement")  # 实现选择是否自动分配
tf.flags.DEFINE_boolean("log_device_placement", False, "Log placement of ops on devices")   #实现选择是否打印日志
...
print("") 
'''
以上为了实现在命令行中选择需要设定的参数来运行程序， 
而不用反复修改源代码中的参数，直接在命令行中进行参数的设定
'''
def preprocess():
    # Data Preparation
    # ==================================================
    ...
    # Load data
    print("Loading data...")
    x_text, y = data_helpers.load_data_and_labels(FLAGS.positive_data_file, FLAGS.negative_data_file) # x_text 词汇表 一个可产生str的迭代器

    # Build vocabulary
    max_document_length = max([len(x.split(" ")) for x in x_text])  
    vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length)
    x = np.array(list(vocab_processor.fit_transform(x_text)))
        #建词典，找索引，不足长度或者不存在的词补0   从x_text学到词汇表 返回矩阵
    ...
    print("Train/Dev split: {:d}/{:d}".format(len(y_train), len(y_dev)))  #比例
...
def train(x_train, y_train, vocab_processor, x_dev, y_dev):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Summaries for loss and accuracy  # 损失函数和准确率的参数保存
            ...
            def train_step(x_batch, y_batch):  #训练函数
                """
            ...
            def dev_step(x_batch, y_batch, writer=None):   # 测试函数
                """
            ...
            for batch in batches:
                ...
                if current_step % FLAGS.evaluate_every == 0:   # 每evaluate_every次执行一次测试
                    print("\nEvaluation:")
